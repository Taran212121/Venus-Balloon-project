...
R_gondola = 0.6         # m      gondola radius (vertical thin cylinder)
...
pend_initial_angle_deg  = 2.0    # deg     uniform initial chain swing angle
pend_initial_rate_deg_s = 0.0    # deg/s   uniform initial chain swing rate
pend_impulse_Ns         = 0.0    # N s     generalized lateral impulse on y1
...
def y_coord(y1, k, theta):
    ...
    if k >= 1:
        ...
        # full tether lengths above current body
        for i in range(1, k):
            y += L_one_tether_pend * np.sin(theta[i])

        # current tether segment CG
        if k < N_p - 1:
            y += 0.5 * L_one_tether_pend * np.sin(theta[k])

    # Gondola
    if k == N_p - 1:

        for i in range(1, N_tether_pendulum + 1):
            y += L_one_tether_pend * np.sin(theta[i])

        y += 0.5 * L_gondola * np.sin(theta[k])
...
def z_coord(k, theta):
    ...
    if k >= 1:
        ...
        if k < N_p - 1:
            z += -0.5 * L_one_tether_pend * np.cos(theta[k])
...
def update(frame):
    ...
    # gondola center (at end of last link)
    yg_center = yc + 0.5 * L_gondola * np.sin(theta[-1])
    zg_center = zc - 0.5 * L_gondola * np.cos(theta[-1])
    ...
    # optional: rotate rectangle with tilt angle
    gondola_patch.angle = np.rad2deg(theta[-1])

    # ---------------------------------
    # TITLE (fixes "t=0 stuck" issues)
    # ---------------------------------
